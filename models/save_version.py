...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.head_type == 'pretrain':
            self.head = Pretrain_Head(self.d_model, self.seq_len)
        elif self.head_type == 'prediction':
            self.head = Forcasting_Head(self.d_model, self.pred_len, self.head_dropout)
...
class Pretrain_Head(nn.Module):
    def __init__(self, seq_len, d_model):
        super().__init__()
        self.head = nn.Linear(d_model * 2, seq_len)
    
    def forward(self, x):
        # [batch_size, channel, d_model * 2]
        # out: [batch_size, channel, seq_len]
        x = self.head(x)
        return x.permute(0, 2, 1)



class Forcasting_Head(nn.Module):
    def __init__(self, d_model, pred_len, head_dropout):
        super().__init__()
